...
export default function DashboardV1() {
  ...
  const emtSupply = bigNumberify("1000000000000000000000000").sub(bigNumberify("598530625016359222222472"));

  const readerAddress = getContract(chainId, "Reader");
  const ammFactoryAddressV2 = getContract(chainId, "AmmFactoryV2");
  const emtAddress = getContract(chainId, "EMT");
  const xemtAddress = getContract(chainId, "XEMT");
  ...
  const { data: pairInfo, mutate: updatePairInfo } = useSWR(
    ...
    {
      fetcher: contractFetcher(library, Reader, [[emtAddress, usdgAddress, xemtAddress, usdgAddress]]),
    }
  );
  ...
  const { data: xemtSupply, mutate: updateXemtSupply } = useSWR(
    ["Dashboard:xemtSupply", chainId, readerAddress, "getTokenSupply", xemtAddress],
    {
  ...
  if (pairInfo && pairInfo.length === 4) {
    emtPairInfo = [pairInfo[0], pairInfo[1]];
    xemtPairInfo = [pairInfo[2], pairInfo[3]];
  }

  let emtPrice;
  let emtMarketCap;
  if (emtPairInfo && emtSupply && emtPairInfo[1] && emtPairInfo[0] && emtPairInfo[0].gt(0)) {
    emtPrice = emtPairInfo[1].mul(PRECISION).div(emtPairInfo[0]);
    emtMarketCap = emtSupply.mul(emtPrice).div(expandDecimals(1, 18));
  }

  let xemtPrice;
  let xemtMarketCap;
  if (xemtPairInfo && xemtSupply && xemtPairInfo[1] && xemtPairInfo[0] && xemtPairInfo[0].gt(0)) {
    xemtPrice = xemtPairInfo[1].mul(PRECISION).div(xemtPairInfo[0]);
    xemtMarketCap = xemtSupply.mul(xemtPrice).div(expandDecimals(1, 18));
  }
      ...
      <div className="Dashboard-token-list-container">
        <div className="Dashboard-token-list Dashboard-list">
          ...
          <div className="Dashboard-token-card App-card">
            <div className="Dashboard-token-title App-card-title">
              <div className="Dashboard-token-title-text">EMT</div>
              <div className="Dashboard-token-title-options">
                <img
                  src={metamaskImg}
                  alt="MetaMask"
            ...
            <div className="Dashboard-token-card-bottom App-card-content">
              <div className="Dashboard-token-info App-card-row">
                ...
                <div>{formatAmount(emtSupply, 18, 0, true)} EMT</div>
              </div>
              <div className="App-card-row">
                <div className="label">
                  <Trans>Price</Trans>
                </div>
                <div>{formatAmount(emtPrice, USD_DECIMALS, 2, true)} USD</div>
              </div>
              <div className="App-card-row">
                <div className="label">
                  <Trans>Market Cap</Trans>
                </div>
                <div>{formatAmount(emtMarketCap, USD_DECIMALS, 0, true)} USD</div>
              </div>
            </div>
          </div>
          <div className="Dashboard-token-card App-card">
            <div className="Dashboard-token-title App-card-title">
              <div className="Dashboard-token-title-text">xEMT</div>
              <div className="Dashboard-token-title-options">
                <img
                  src={metamaskImg}
                  alt="MetaMask"
            ...
            <div className="Dashboard-token-card-bottom App-card-content">
              <div className="Dashboard-token-info App-card-row">
                ...
                <div>{formatAmount(xemtSupply, 18, 0, true)} xEMT</div>
              </div>
              <div className="App-card-row">
                <div className="label">
                  <Trans>Price</Trans>
                </div>
                <div>{formatAmount(xemtPrice, USD_DECIMALS, 2, true)} USD</div>
              </div>
              <div className="App-card-row">
                <div className="label">
                  <Trans>Market Cap</Trans>
                </div>
                <div>{formatAmount(xemtMarketCap, USD_DECIMALS, 0, true)} USD</div>
              </div>
            </div>
          </div>
          {infoTokens &&
