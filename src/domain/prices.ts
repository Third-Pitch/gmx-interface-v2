...
export async function getLimitChartPricesFromStats(chainId, symbol, period, limit = 1) {
  ...
  const url = `${EDDX_STATS_API_URL}/candles/${symbol}?preferableChainId=${chainId}&period=${period}&limit=${limit}`;
...
export async function getChartPricesFromStats(chainId, symbol, period) {
  ...
  const url = `${EDDX_STATS_API_URL}/candles/${symbol}?preferableChainId=${chainId}&period=${period}&from=${from}&preferableSource=fast`;
