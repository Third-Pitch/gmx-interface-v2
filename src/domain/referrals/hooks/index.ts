...
import { BASE, AVALANCHE, SUPPORTED_CHAIN_IDS } from "config/chains";
...
export function useUserCodesOnAllChain(account) {
  ...
  useEffect(() => {
    async function main() {
      const [baseCodes, avalancheCodes] = await Promise.all(
      ...
      const [codeOwnersOnAvax = [], codeOwnersOnBase = []] = await Promise.all([
        getCodeOwnersData(AVALANCHE, account, baseCodes),
      ...
      setData({
        ...
        [AVALANCHE]: codeOwnersOnBase.reduce((acc, cv) => {
          acc[cv.code] = cv;
          return acc;
        }, {} as any),
      });
