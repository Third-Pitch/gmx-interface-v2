...
import { EmFees } from "components/Synthetics/EmSwap/EmFees/EmFees";
...
import { EmSwapFees, useAvailableTokenOptions } from "domain/synthetics/trade";
...
import { EmConfirmationBox } from "../EmConfirmationBox/EmConfirmationBox";
...
import { getCommonError, getEmSwapError } from "domain/synthetics/trade/utils/validation";
import { helperToast } from "lib/helperToast";
import { useSafeState } from "lib/useSafeState";
import { useHistory, useLocation } from "react-router-dom";
import "./EmSwapBox.scss";
...
export function EmSwapBox(p: Props) {
  ...
  const operationLabels = {
    [Operation.Deposit]: t`Buy EM`,
    [Operation.Withdrawal]: t`Sell EM`,
  };
  ...
  const submitState = useMemo(() => {
    ...
    const swapError = getEmSwapError({
      isDeposit,
      marketInfo,
      marketToken,
    ...
    return {
      text: isDeposit ? t`Buy EM` : t`Sell EM`,
      onSubmit: () => {
  ...
  return (
    <div className={`App-box EmSwapBox`}>
      <Tab
      ...
      <Tab
        ...
        className="EmSwapBox-asset-options-tabs"
        type="inline"
        option={mode}
        onChange={setMode}
      />

      <div className={cx("EmSwapBox-form-layout", { reverse: isWithdrawal })}>
        <BuyInputSection
        ...
        >
          <div className="selected-token">EM</div>
        </BuyInputSection>
      </div>

      <div className="EmSwapBox-info-section">
        <ExchangeInfoRow
