...
export function EmConfirmationBox({
  isVisible,
  marketToken,
  longToken,
  shortToken,
  ...
  const submitButtonState = (function getSubmitButtonState() {
    ...
    if (tokensToApprove.length > 0 && marketToken) {
      const symbols = tokensToApprove.map((address) => {
        return address === marketToken.address ? "EM" : getTokenData(tokensData, address)!.symbol;
      });
    ...
    const text = t`Confirm ${operationText} ${formatTokenAmount(marketTokenAmount, marketToken?.decimals)} EM`;
  ...
  return (
    <div className="Confirmation-box EmConfirmationBox">
      <Modal isVisible={isVisible} setIsVisible={onClose} label={t`Confirm ${operationText}`} allowContentTouchMove>
        {isVisible && (
          <>
            <div className={cx("Confirmation-box-main EmConfirmationBox-main")}>
              {isDeposit && (
                <>
                ...
                {tokensToApprove.map((address) => (
                  <div key={address}>
                    <ApproveTokenButton
                      ...
                      tokenSymbol={address === marketToken?.address ? "EM" : getToken(chainId, address).symbol}
                      spenderAddress={routerAddress}
                    />
                  </div>
                ))}
