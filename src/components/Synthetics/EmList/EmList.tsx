...
import { Operation } from "../EmSwap/EmSwapBox/EmSwapBox";
import "./EmList.scss";
...
export function EmList({ hideTitle, marketTokensData, marketsInfoData, tokensData, marketsTokensAPRData }: Props) {
          ...
          {!hideTitle && (
            <>
              <div className="App-card-title">
                <Trans>EM ({getChainName(chainId)})</Trans>
              </div>
              ...
              {marketTokens.map((token) => {
                ...
                return (
                  <tr key={token.address}>
                    ...
                    <td>
                      {formatTokenAmount(token.balance, token.decimals, "EM", { useCommas: true })}
                      <br />({formatUsd(convertToUsd(token.balance, token.decimals, token.prices?.minPrice)) || "..."})
                    </td>

                    <td>{apr ? `${formatAmount(apr, 2, 2)}%` : "..."}</td>

                    <td className="EmList-last-column">
                      {formatTokenAmount(totalSupply, token.decimals, "EM", {
                        useCommas: true,
                      })}
                      <br />({formatUsd(totalSupplyUsd)})
                    </td>

                    <td className="EmList-actions">
                      <Button
                ...
                <div className="App-card" key={token.address}>
                  ...
                  <div className="App-card-content">
                    ...
                    <div className="App-card-row">
                      ...
                      <div>
                        {formatTokenAmount(token.balance, token.decimals, "EM")} (
                    ...
                    <div className="App-card-row">
                      ...
                      <div>
                        {" "}
                        {formatTokenAmount(totalSupply, token.decimals, "EM")} ({formatUsd(totalSupplyUsd)})
                      </div>
                    </div>
                    <div className="App-card-divider"></div>
